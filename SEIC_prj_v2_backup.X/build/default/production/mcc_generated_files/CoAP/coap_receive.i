# 1 "mcc_generated_files/CoAP/coap_receive.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "mcc_generated_files/CoAP/coap_receive.c" 2
# 40 "mcc_generated_files/CoAP/coap_receive.c"
...
# 40 "mcc_generated_files/CoAP/coap_receive.c" 2
...
# 41 "mcc_generated_files/CoAP/coap_receive.c" 2
...
# 42 "mcc_generated_files/CoAP/coap_receive.c" 2
...
# 43 "mcc_generated_files/CoAP/coap_receive.c" 2
...
# 44 "mcc_generated_files/CoAP/coap_receive.c" 2
...
# 45 "mcc_generated_files/CoAP/coap_receive.c" 2
...
# 46 "mcc_generated_files/CoAP/coap_receive.c" 2
...
static char recvdClientToken[(8)];

static _Bool COAP_ReceiveNONMessage(int16_t length);
static _Bool COAP_ReceiveCONMessage(int16_t length);
static _Bool COAP_ReceiveACKMessage(int16_t length);
static _Bool COAP_ReceiveRSTMessage(int16_t length);
...
static int8_t COAP_ReadRecivedHeaderAndToken(void)
{
    ...
    ETH_ReadBlock((uint8_t*)&coapReceiveHeader,sizeof(coapReceiveHeader));
    ret += sizeof(coapReceiveHeader);

    if(coapReceiveHeader.ver == 1)
    {
        coapRecv.messageID = ETH_Read16();
        ret += 2;

        coapRecv.code = coapReceiveHeader.code;
        coapRecv.destIP = (ipv4Header.srcIpAddress);
        coapRecv.destPort = (destPort);
        coapRecv.srcPort = udpHeader.dstPort;
        coapRecv.type = coapReceiveHeader.t;
        if(coapReceiveHeader.tkl > (8))
        {
            return -1;
        }
        else if(coapReceiveHeader.tkl > 0 && coapReceiveHeader.tkl <= (8))
        {
            ETH_ReadBlock(coapRecv.token,coapReceiveHeader.tkl);
...
_Bool COAP_MessageResponse(coapReceive_t *ptr)
{
    ...
    if(ret == (1))
    {
        ETH_WriteBlock((char*)&coapReceiveHeader,sizeof(coapReceiveHeader));
        ETH_Write16(ptr->messageID);
        if(coapReceiveHeader.tkl > 0)
        {
            ETH_WriteBlock((char*)ptr->token,coapReceiveHeader.tkl);
        }
        return 1;
    }
